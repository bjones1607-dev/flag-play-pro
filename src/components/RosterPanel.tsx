...
export function RosterPanel() {
  ...
  const update = (next: Player[]) => { setPlayers(next); savePlayers(next); };
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        <Input placeholder="#" value={num} onChange={(e) => setNum(e.target.value)} className="w-16" maxLength={3} />
        <Button onClick={add} size="icon" variant="default"><UserPlus className="h-4 w-4" /></Button>
      </div>
      <div className="space-y-2 max-h-72 overflow-y-auto">
        {players.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-4">No players yet. Add your roster above.</p>
        )}
        {players.map((p) => (
          <div key={p.id} className="flex items-center gap-3 bg-secondary rounded-lg p-3">
            ...
            <Button size="icon" variant="ghost" onClick={() => update(players.filter(x => x.id !== p.id))}>
